...
--     | (r1 ^. time) == (r2 ^. time) = (r1 ^. action) `compare` (r2 ^. action)
--     | otherwise = (r1 ^. action) `compare` (r2 ^. action)

instance Ord Record where
  r1 `compare` r2 
    | (r1 ^. action == r2 ^. action) = (r2 ^. date) `compare` (r1 ^. date)
    | otherwise = (r1 ^. time) `compare` (r2 ^. time)

day4 :: IO Text
day4 = do
  input1 <- T.lines <$> readFile "input_day4_1.txt"
  mapM_ print $ (map Utils.showT . sort . map parseRecord) input1
  pure ""
    where 
      sortR :: [Record] -> [Record]
      sortR = undefined
      -- sortR = (sortBy (\r1 r2 -> r1 ^. date `compare` r2 ^. date))
